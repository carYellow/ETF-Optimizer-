...
class RobustTrainTestSplit:
    ...
    def temporal_train_test_split(self, df: pd.DataFrame, 
        ...
        n_train = n_samples - n_test - self.gap_days
        
        # Ensure minimum training samples
        if n_train < min_train_samples:
            raise ValueError(f"Not enough training samples: {n_train} < {min_train_samples}")
    ...
    def validate_no_lookahead(self, train_df: pd.DataFrame, test_df: pd.DataFrame) -> bool:
        ...
        if train_end >= test_start:
            raise ValueError(f"Lookahead bias detected! Train end ({train_end}) >= Test start ({test_start})")
        
        # Check gap
        gap = (test_start - train_end).days
        if gap < self.gap_days:
            raise ValueError(f"Insufficient gap! {gap} days < required {self.gap_days} days")
        
        # Check for data leakage in features
        # This would require checking if any features use future information
